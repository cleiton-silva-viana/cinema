...
describe("FailureMessageProvider", () => {
  ...
  it("deve carregar mensagens do arquivo JSON", () => {
    // Act
    const testErrorConfig = provider.getMessageConfig(
      "TEST_ERROR" as FailureCode,
    );
  ...
  it("deve retornar a configuração de mensagem correta para código de erro existente", () => {
    // Act
    const notFoundConfig = provider.getMessageConfig(
      "NOT_FOUND" as FailureCode,
    );
    ...
    expect(notFoundConfig?.template.pt).toBe(
      "O recurso solicitado não foi encontrado",
    );
    expect(notFoundConfig?.template.en).toBe(
      "The expected resource is not found",
    );
  ...
  it("deve retornar um erro genérico caos o erro não seja identificado", () => {
    // Act
    const genericError = provider.getMessageConfig(
      "UNKNOWN_ERROR_CODE" as FailureCode,
    );
