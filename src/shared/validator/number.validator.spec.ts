...
describe("NumberValidator", () => {
  ...
  describe("Fluxo de validação", () => {
    it("deve ignorar as validações quando if for false", () => {
      ...
      // Act
      new NumberValidator(value).failures(failures).if(false).isPositive();
    ...
    it("deve continuar a validação quando o fluxo for continue", () => {
      ...
      expect(failures[0].code).toBe(FailureCode.VALUE_NOT_POSITIVE);
      expect(failures[1].code).toBe(FailureCode.VALUE_NOT_INTEGER);
      expect(failures[2].code).toBe(FailureCode.VALUE_OUT_OF_RANGE);
    });
  });
});
