...
describe('Password', () => {
  describe('create', () => {
    ...
    describe('deve falhar ao criar uma senha inválida', () => {
      const failureCases = [
        ...
        {
          password: 'thispasswordiswaytoolongandshouldfailthispasswordiswaytoolongandshouldfail',
  ...
  describe('hydrate', () => {
    it('deve criar um objeto Password sem validação com hash válido', () => {
      // Arrange
      const hashedPassword = '$argon2id$v=19$m=65536,t=3,p=4$c29tZXNhbHQ$R0p4cmcvN3p2SVdSc0hGZQ'
    ...
    it('deve falhar ao tentar hidratar com valor nulo', () => {
      expect(() => Password.hydrate(null as unknown as string)).toThrow(TechnicalError)
    })

    it('deve falhar ao tentar hidratar com valor indefinido', () => {
      expect(() => Password.hydrate(undefined as unknown as string)).toThrow(TechnicalError)
    })
  })
  ...
  describe('Compare', () => {
    ...
    beforeAll(async () => {
      ...
      const credentialPasswordResult = await Password.create(credentialPassword)
    ...
    it('deve retornar verdadeiro quando corresponde ao hash', async () => {
      // Act
      const result = await Password.Compare(credentialHashedPassword, credentialPassword)
