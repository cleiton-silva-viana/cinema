...
describe('Email', () => {
    describe('create', () => {
        describe('should create a valid', () => {
            const successCases = [
                ...
                { email: 'user@subdomain.example.co.uk', scenario: 'with subdomain and country TLD' }
            ];
        ...
        describe('should fail to create an invalid', () => {
            const failureCases = [
                { email: null as unknown as string, scenario: 'when email is null', errorCodeExpected: 'FIELD_CANNOT_BE_NULL' },
                { email: undefined as unknown as string, scenario: 'when email is undefined', errorCodeExpected: 'FIELD_CANNOT_BE_NULL' },
                { email: '', scenario: 'when email is empty', errorCodeExpected: 'FIELD_CANNOT_BE_EMPTY' },
                { email: 'plainaddress', scenario: 'when email has no @ symbol', errorCodeExpected: 'EMAIL_WITH_INVALID_FORMAT' },
                { email: '@missingusername.com', scenario: 'when email has no username', errorCodeExpected: 'EMAIL_WITH_INVALID_FORMAT' },
                { email: 'user@', scenario: 'when email has no domain', errorCodeExpected: 'EMAIL_WITH_INVALID_FORMAT' },
                { email: 'user@.com', scenario: 'when email has no domain name', errorCodeExpected: 'EMAIL_WITH_INVALID_FORMAT' },
                { email: 'user@domain,com', scenario: 'when email has invalid character in domain', errorCodeExpected: 'EMAIL_WITH_INVALID_FORMAT' }
            ];
    ...
    describe('equal', () => {
        ...
        it('should return false when comparing with non-Email object', () => {
            ...
            // Assert
            expect(result.value.equal(notEmailObject as unknown as Email)).toBe(false);
        });
    });
});