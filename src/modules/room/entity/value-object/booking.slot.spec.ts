...
describe('BookingSlot', () => {
  ...
  describe('create', () => {
    ...
    const VALID_END_TIME = new Date(VALID_START_TIME.getTime() + 1000 * 60 * 120) // 2 horas depois do startTime (duração de 120 minutos)
    ...
    describe('create', () => {
      ...
      describe('deve falhar ao criar um objeto BookingSlot inválido', () => {
        ...
        const shortDurationEndTime = new Date(VALID_START_TIME.getTime() + 1000 * 60 * 29) // 29 minutos de duração
        const longDurationScreeningEndTime = new Date(VALID_START_TIME.getTime() + 1000 * 60 * 361) // 361 minutos de duração
        const longDurationCleaningEndTime = new Date(VALID_START_TIME.getTime() + 1000 * 60 * 121) // 121 minutos de duração
        const longDurationMaintenanceEndTime = new Date(VALID_START_TIME.getTime() + 1000 * 60 * (3 * 24 * 60 + 1)) // 3 dias + 1 minuto de duração
        const shortDurationExitTimeEndTime = new Date(VALID_START_TIME.getTime() + 1000 * 60 * 14) // 14 minutos de duração (mínimo é 15)
        const longDurationExitTimeEndTime = new Date(VALID_START_TIME.getTime() + 1000 * 60 * 31) // 31 minutos de duração (máximo é 30)
        const shortDurationEntryTimeEndTime = new Date(VALID_START_TIME.getTime() + 1000 * 60 * 14) // 14 minutos de duração (mínimo é 15)
        const longDurationEntryTimeEndTime = new Date(VALID_START_TIME.getTime() + 1000 * 60 * 21) // 21 minutos de duração (máximo é 20)
        ...
        describe('falhas com campo específico', () => {
          const failureCases = [
            ...
            {
              scenario: 'quando screeningUID é nulo e type é igual a ENTRY_TIME',
              props: {
          ...
          failureCases.forEach(({ scenario, props, expectedCode }) => {
            it(scenario, () => {
              // Arrange
              const input = {
        ...
        describe('falhas de duração sem campo específico', () => {
          const durationFailureCases = [
            {
              scenario: 'quando a duração para SCREENING é muito curta (< 30 min)',
              props: {
            ...
            {
              scenario: 'quando a duração para SCREENING é muito longa (> 360 min)',
              props: {
            ...
            {
              scenario: 'quando a duração para CLEANING é muito longa (> 120 min)',
              props: {
            ...
            {
              scenario: 'quando a duração para MAINTENANCE é muito longa (> 3 dias)',
              props: {
            ...
            {
              scenario: 'quando a duração para EXIT_TIME é muito curta (< 15 min)',
              props: {
            ...
            {
              scenario: 'quando a duração para EXIT_TIME é muito longa (> 30 min)',
              props: {
            ...
            {
              scenario: 'quando a duração para ENTRY_TIME é muito curta (< 15 min)',
              props: {
            ...
            {
              scenario: 'quando a duração para ENTRY_TIME é muito longa (> 20 min)',
              props: {
            ...
            },
            {
              ...
              props: {
                ...
                endTime: new Date(Date.now() + 24 * 60 * 60 * 1000 - 60 * 60 * 1000), // 1 hora antes
              },
              expectedCode: FailureCode.DATE_WITH_INVALID_SEQUENCE,
            },
            {
              scenario: 'Deve falhar quando screeningUID for null para tipo SCREENING',
              props: {
                screeningUID: null as any,
              },
              expectedCode: FailureCode.MISSING_REQUIRED_DATA,
    ...
    describe('hydrate', () => {
      ...
      it('deve criar um objeto BookingSlot sem validação', () => {
        // Act
        const result = BookingSlot.hydrate('uid', mockScreeningUIDString, VALID_START_TIME, VALID_END_TIME, VALID_TYPE)
