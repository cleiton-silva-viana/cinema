  ...
  describe('Static Methods', () => {
    ...
    describe('hydrate', () => {
      ...
      it('deve restaurar uma imagem com os dados fornecidos', () => {
        ...
        expect(image.title.content(SupportedLanguage.PT)).toBe(VALID_PARAMS.title.text)
        expect(image.description.content(SupportedLanguage.PT)).toBe(VALID_PARAMS.description.text)
    ...
    describe('update', () => {
      ...
      describe('deve atualizar corretamente as propriedades', () => {
        it('deve atualizar a propriedade title corretamente', () => {
          ...
          expect(result.title.content(SupportedLanguage.EN)).toBe(params.title![0].text)
          expect(result.title.content(SupportedLanguage.PT)).toBe(params.title![1].text)
        })
        ...
        it('deve atualizar a propriedade description corretamente', () => {
          ...
          expect(result.description.content(SupportedLanguage.EN)).toBe(params.description![0].text)
          expect(result.description.content(SupportedLanguage.PT)).toBe(params.description![1].text)
        })
      ...
      it('deve falhar ao não receber dados para atualização', () => {
        // Act
        const result = validateAndCollect(instance.update(null as any), failures)
