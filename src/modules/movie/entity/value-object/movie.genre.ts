...
export class MovieGenre {
  ...
  public static hydrate(genres: string[]): MovieGenre {
    TechnicalError.if(isNull(genres), 'genres cannot be null or undefined')
    TechnicalError.if(genres.length === 0, 'genres cannot be empty')
  ...
  public static create(genres: Genre[] | string[]): Result<MovieGenre> {
    if (!genres || genres.length === 0) {
      return failure([{
        code: GenreCodes.INVALID_GENRE_COUNT,
        details: { 
  ...
  private static createFromEnums(genres: Genre[]): Result<MovieGenre> {
    ...
    Assert.untilFirstFailure(
      failures,
      not.null(genres, GenreCodes.INVALID_GENRE_COUNT, { minGenres: MovieGenre.MIN_GENRES }),
      is.true([...new Set(genres)].length === genres.length, GenreCodes.DUPLICATE_GENRES),
      is.between(genres, MovieGenre.MIN_GENRES, MovieGenre.MAX_GENRES, GenreCodes.INVALID_GENRE_COUNT, {
        minGenres: MovieGenre.MIN_GENRES,
  ...
  public static createFromStrings(genreStrings: string[]): Result<MovieGenre> {
    ...
    Assert.untilFirstFailure(
      ...
      not.null(genreStrings, GenreCodes.INVALID_GENRE_COUNT),
      is.greaterOrEqualTo(genreStrings?.length, MovieGenre.MIN_GENRES, GenreCodes.INVALID_GENRE_COUNT),
      is.lessOrEqualTo(genreStrings?.length, MovieGenre.MAX_GENRES, GenreCodes.INVALID_GENRE_COUNT)
    );
    ...
    genreStrings.forEach((genreString) => {
      ...
      if (uniqueGenres.has(upperCaseGenre as Genre)) {
        failures.push({
          code: GenreCodes.DUPLICATE_GENRES,
          details: {
